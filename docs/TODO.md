...
- [ ] FEA001: 新增支持分享单个页面的选项，用户可以在预览面板点击分享按钮，生成分享链接
  - 分享链接包含当前页面的 url(hash后的)，用户可以分享给其他人，其他人可以通过点击链接访问当前页面
  - 分享链接过期时间默认为1小时，过期后需要重新生成链接
  - 分享链接只能在当前项目内有效，不能跨项目使用，且只能查看当前页面，不显示目录列表
  - 要支持这个功能，可能需要简单的认证机制，例如使用 basicAuth/token 等技术
  - server 需要新增支持配置 分享host 地址(内网/域名地址)
  - 分享 server 是不是要起一个单独的 server port?
...
- [x] 增强 文件内容搜索支持 `path:dirname/ keywords...` 限制搜索的dir path必须包含给定path,提高准确性
  - 搜索框下面，添加一些简要搜索使用帮助说明
- [x] FEA002: 现在已经有全局的 projects 信息了，能否实现类似 --global 时，启动的server 默认显示的是项目卡片列表
  - 此时不关心从哪个目录启动的 server
  - 卡片上会显示项目的一些信息，名称，地址等
  - 点击项目卡片后可以直接进入项目目录查看文档，跟在项目里启动一样(搜索等都在项目下)
  - 内容区域顶部新增显示个 topbar, 可以点击回到主页(项目列表) 同时显示当前项目简略信息(路径)

## UI 问题

- [x] 在预览面板里点击相对文档链接会报错找不到文件，实际是存在的，需要修复。
- [x] 预览面板里点击内部链接，就在预览面板打开即可
- [x] UI 优化，使用 frontend-design 优化下现在的UI设计
  - 主内容和预览面板内容 下方之前有留空白的，现在没有了，底部加一些空白
- [x] UI005: compact 模式时，收起侧栏后，只能看到上半部分显示
- [x] UI006: toc-middle, toc-right 时，toc 收起显示不好看，会遮住滚动条
  - 改为收起后为下方显示 toc图标(稍大一点)

## UI/UX 调整

- [x] UX001: 内容搜索框，在输入 path:xx 还未输入keyword时也会触发搜索API调用
- [x] UX002: 推送的文档变动消息，宽度加宽 文档可以点击跳转页面
