...
**Architecture:** 保持单一 Files DOM、单一 TOC DOM、单一正文 DOM，不复制目录树或 TOC。`web/template.html` 将页面主区域拆成 `.app-shell` 下的 `.files-pane`、`.toc-pane`、`.content-wrapper` 三个同级 pane；`web/src/style/app.css` 使用 CSS Grid 和 `html[data-layout]` 切换 `compact`、`toc-middle`、`toc-right`，不使用 fixed TOC 或 JS 搬 DOM。`web/src/preferences.ts` 负责 layout 偏好读写和清除；`web/src/layout-mode.ts` 负责设置面板控件状态和页面级 dataset。
...
在 `web/src/style/app.css` 增加 `app-shell`、pane 和正文内部 wrapper 的基础规则。此时只做到 compact 视觉兼容，不实现三栏切换：
...
describe('layout CSS modes', () => {
    test('defines desktop rules for toc-middle and toc-right', () => {
        ...
        expect(cssText).toContain('"files content toc"');
    });
...
Expected: FAIL，CSS 还没有三栏 grid 规则。

- [ ] **Step 3: 实现桌面三栏规则**

采用独立 TOC pane + `.app-shell` CSS Grid 策略。不要使用 fixed TOC、不要复制 TOC DOM、不要用 JS 在 compact 和三栏之间移动 TOC。DOM 在 Task 3 已经保证 `.files-pane`、`.toc-pane`、`.content-wrapper` 是 `.app-shell` 的直接子项，三种布局只通过 grid areas 调整视觉位置。
...
@media (min-width: 1024px) {
    ...
    html[data-layout="toc-right"] .app-shell {
        ...
        grid-template-rows: minmax(0, 1fr);
        grid-template-areas: "files content toc";
    }
...
如果为了降低改动范围需要短期保留旧规则，必须用 `.app-shell` 新规则在桌面和移动端都明确覆盖，避免 fixed sidebar 与 grid 同时生效。
...
@media (min-width: 1024px) {
    ...
    html[data-layout="toc-right"] body.sidebar-collapsed .app-shell {
        grid-template-columns: var(--sidebar-collapsed-width) minmax(0, 1fr) var(--toc-width);
    }
...
因为 TOC 已不在 `.sidebar-panels` 内，旧的 `.sidebar-collapsed .sidebar-panels { display: none; }` 不应再影响 TOC。实现后必须确认 `.sidebar-icons` 在三栏模式 collapse 后仍可点击恢复 files。
...
- `toc-middle`/`toc-right`: preview 打开时隐藏 `.toc-pane`，`.app-shell` grid 变为 files + content。
...
@media (min-width: 1024px) {
    ...
    html[data-layout="toc-right"] body.sidebar-collapsed.preview-active .app-shell {
        grid-template-columns: var(--sidebar-collapsed-width) minmax(0, 60%);
    }
}
```
...
- `.toc-pane` 不作为独立列展示，可隐藏或放到正文后方；本期推荐隐藏，避免覆盖正文。
